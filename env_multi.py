...
# Track best section times (for 200 segments)
best_section_times = {}  
...
def apply_action(action_index):
    ...
    Map the provided action index to controller commands with the new action modifications:
      ...
      6: Special action (remains unchanged).
    ...
    if action_index == 0:
        ...
        nunchuck_action = {"StickX": -0.35, "StickY": 0.0, "Z": False}
        controller.set_wii_nunchuk_buttons(0, nunchuck_action)
        return
    elif action_index == 2:
...
def reset_environment(initial=False):
    ...
    global prev_drift_value, current_action, prev_action, episode_accum, last_speed, best_section_times
    global current_section_start_time, last_checkpoint_progress
    ...
    if not initial:
        ...
        if user == "Zach":
            ...
            # Luigi Circuit
            # if reset_choice == 1:
            #     savestate.load_from_file(r"F:\MKWii_Capstone_Project\UPDATED_MKWii_Capstone\Mario-Kart-Wii-AI\funky_flame_luigi_savestate.sav")
            # elif reset_choice == 2:
            #     savestate.load_from_file(r"F:\MKWii_Capstone_Project\UPDATED_MKWii_Capstone\Mario-Kart-Wii-AI\funky_flame_luigi_savestate2.sav")
            # elif reset_choice == 3:
            #     savestate.load_from_file(r"F:\MKWii_Capstone_Project\UPDATED_MKWii_Capstone\Mario-Kart-Wii-AI\funky_flame_luigi_savestate3.sav")
        elif user == "Nolan":
            if reset_choice == 1:
    ...
    if command.get("command") == "step":
        ...
        for i in range(frame_skip):
            ...
            if terminal:
                ...
                print(f"Total Episode Reward: {final_episode_reward}")
