...
class BTRAgent:
    ...
    def choose_action(self, observation, debug=False):
        """
        Actor‑thread call.  Protected by _net_lock so that resetting noisy
        layers or reading weights never races with the learner thread.
        """
        with self._net_lock, torch.no_grad():
            ...
            if self.noisy and not self.eval_mode:
                self.reset_noise(self.net)          # safe now
            ...
            if debug:
                ...
                logging.info("DEBUG: Action chosen based on max Q: %s",
                             T.argmax(qvals, dim=1).cpu().numpy())

            x = T.argmax(qvals, dim=1).cpu()        # greedy

        # ϵ‑greedy noise can execute outside the lock
    ...
    def store_transition(self, state, action, reward, next_state, done, stream, prio=True):
        """
        Called by the actor thread; lock the replay buffer so the learner
        thread can't sample while we append.
        """
        # convert only if necessary
        if not isinstance(state,       np.ndarray): state       = to_numpy(state)
        if not isinstance(next_state,  np.ndarray): next_state  = to_numpy(next_state)
    ...
    def learn_call(self):
        """
        One gradient‐update step executed by the learner thread.
        Locks:
          · _memory_lock guards PER append/sample/priority‑update.
          · _net_lock    guards network parameter reads / writes
                         (incl. noisy‑layer reset, forward, backward, optimiser).
        """
        # -----------------------------------------------------------------
        # 0. Return early if we are still in burn‑in.
        # -----------------------------------------------------------------
        if self.env_steps < self.min_sampling_size:
            return

        # -----------------------------------------------------------------
        # 1. Anneal PER β (no lock needed) and sample batch (memory lock).
        # -----------------------------------------------------------------
        if self.per and self.per_beta_anneal:
            self.memory.beta = min(self.memory.beta + self.priority_weight_increase, 1)

        with self._memory_lock:
            idxs, states, actions, rewards, next_states, dones, weights = \
                self.memory.sample(self.batch_size)

        # -----------------------------------------------------------------
        # 2. Forward, loss, backward, optimiser (net lock).
        # -----------------------------------------------------------------
        with self._net_lock:
            # make target net noisy weights fresh every grad‑step
            if self.noisy:
            ...
            with torch.amp.autocast(device_type=self.device):
                # IQN + Munchausen path (your current configuration)
                if self.iqn and self.munchausen:
                    # ---- target calculation (no grad) -------------------
                    with torch.no_grad():
                        Q_targets_next, _ = self.tgt_net(next_states)
                        q_t_n       = Q_targets_next.mean(dim=1)                 # [B, A]
                        actions_u   = actions.unsqueeze(1)                       # [B,1]
                        rewards_u   = rewards.unsqueeze(1)                       # [B,1]
                        dones_u     = dones.unsqueeze(1)                         # [B,1]
                        if self.per:
                            weights = weights.unsqueeze(1)                       # [B,1]

                        logsum      = torch.logsumexp(
                                            (q_t_n - q_t_n.max(1)[0].unsqueeze(-1))
                                            / self.entropy_tau, 1).unsqueeze(-1)

                        tau_log_pi_next = (q_t_n - q_t_n.max(1)[0].unsqueeze(-1)
                                           - self.entropy_tau * logsum).unsqueeze(1)

                        pi_target   = torch.softmax(q_t_n / self.entropy_tau, dim=1).unsqueeze(1)
                        Q_target    = (self.gamma ** self.n *
                                       (pi_target * (Q_targets_next - tau_log_pi_next) *
                                        (~dones_u.unsqueeze(-1))).sum(2)).unsqueeze(1)

                        q_k_target  = self.net.qvals(states)
                        v_k_target  = q_k_target.max(1)[0].unsqueeze(-1)
                        tau_log_pik = (q_k_target - v_k_target -
                                       self.entropy_tau *
                                       torch.logsumexp((q_k_target - v_k_target) /
                                                       self.entropy_tau, 1).unsqueeze(-1))
                        munchausen_addon  = tau_log_pik.gather(1, actions_u)
                        munchausen_reward = (rewards_u +
                                             self.alpha * torch.clamp(munchausen_addon,
                                                                      min=self.lo, max=0)).unsqueeze(-1)
                        Q_targets   = munchausen_reward + Q_target               # [B,1,1]

                    # ---- online Q, loss ---------------------------------
                    q_k, taus      = self.net(states)                           # [B,T,A], [B,T,1]
                    Q_expected     = q_k.gather(2, actions_u.unsqueeze(-1)
                                                 .expand(self.batch_size, self.num_tau, 1))
                    td_error       = Q_targets - Q_expected                     # [B,T,1]
                    loss_v         = td_error.abs().sum(dim=1).mean(dim=1).data

                    huber_l        = calculate_huber_loss(td_error, 1.0, self.num_tau)
                    quantile_l     = (taus - (td_error.detach() < 0).float()).abs() * \
                                     huber_l / 1.0
                    loss           = quantile_l.sum(dim=1).mean(dim=1, keepdim=True)

                    if self.per:
                        loss = loss * weights.to(self.net.device)
                    loss = loss.mean()
                else:
                    raise NotImplementedError("Only IQN + Munchausen path shown here.")

            # backward + optimiser (still inside net lock)
        ...
        # -----------------------------------------------------------------
        # 3. Priority update (memory lock) & bookkeeping.
        # -----------------------------------------------------------------
        with self._memory_lock:
        ...
        if self.grad_steps % 10_000 == 0:
            print(f"Completed {self.grad_steps} gradient steps")
